...
class Gaussian1D(Model):
    ...
    def getFuncAndGradient(self, mu):
        pass
        # def gaussian_fom_compute_output(fom, mu):
        #     mu = np.atleast_2d(mu)
        #     value =  - np.exp(-mu[0,0]**2) - np.exp( - (mu[0,0] - 1)**2)
        #     der = 2*mu[0,0]* np.exp(-mu[0,0]**2) + (2*(mu[0,0] - 1)) * np.exp( - (mu[0,0] - 1)**2)
        #     return value, np.atleast_2d(der).reshape(1,-1)

        # def compute_gaussian_RKHS_norm(mu, kernel, fom, parameter_space, amount, TR_parameters, target_values=None, grad_target_values=None):
#     mu = np.atleast_2d(mu)

#     if mu[0, -1] > 1/np.sqrt(2):
#         kernel_width = mu[0, -1]
...
class twoDStuff(Model): 
    def __init__(self):
        ...
        self.parameter_space = self.fom.parameters.space(0, np.pi)
